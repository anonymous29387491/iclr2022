...
parser.add_argument('--batch_size', type = int,default=512, help='batch size for black box forward pass')

parser.add_argument('--baseline', type = str,default='mean')
parser.add_argument('--method_name', type = str,default='shapleytaylor')
...
#from MNIST.MLP_baseline import NN_MNIST
save_path = './Files/results_attribution/MNIST_%s_%s' % (str(args.num_superpixels), args.method_name)
...
method = 'shapleytaylor'
Explainer = Image_Explainer(binary_pred = False, model_path = model_path, baseline = baseline, num_superpixels= num_superpixels, sp_mapping = sp_mapping, dataset = dataset, m = m, method = method)
#######################
# Dataset Iterator
