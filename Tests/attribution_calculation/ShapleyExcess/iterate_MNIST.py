...
for idx, (x, target) in batch_iterator:
    ...
    shapley_values = explainer.shap_values(x_, silent = True, l1_reg = False)

    for i in range(n_feat):
        for j in range(i+1, n_feat):
            model_eval.init_baseline(x, num_superpixels = args.num_superpixels, sp_mapping = sp_mapping, j = j, i = i)
