...
explainer_ss = Shapley_Sampling(value_function = model_eval, m = 1000, baseline = 'zero')
...
for idx, batch in batch_iterator:
    ...
    for i in range(n_feat):
        for j in range(i+1, n_feat):
            ...
            shapley_ij,_ = explainer_ss._pairwise_shapley(x_, feat_i = i)

            matrix[i, j] = 0.5 * (shapley_ij.item() - shapley_values[0,i] - shapley_values[0,j])
