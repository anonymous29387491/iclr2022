...
for idx, (x, target) in batch_iterator:
    ...
    model_eval.init_baseline(tensor2numpy(x), args.num_superpixels, superpixel_to_mask)
    explainer = shap.KernelExplainer(model_eval, x_train)
    shapley_values = explainer.shap_values(np.ones(args.num_superpixels))
    ...
    matrix_list.append(np.zeros((2,2)))
