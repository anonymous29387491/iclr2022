...
from shapley_value_functions import eval_XGB
# load model
...
# if the model is not a NN, introduce an approximating NN
MLP_params = {
    'hidden_units': 50,
    'num_layers': 3,
...
model_nn = MLP(**MLP_params).to(device)
model_nn.train()
train_blackbox = True
...
dataset_test = census(data_path = data_path, train = False)
...
dataset_train = census(data_path = data_path, train = True)
